...
class RecurrentActorCriticPolicy(ActorCriticPolicy):
    ...
    def evaluate_actions_w_lstm_state(
        self, obs: th.Tensor, actions: th.Tensor, lstm_states: RNNStates, episode_starts: th.Tensor
    ) -> Tuple[th.Tensor, th.Tensor, th.Tensor]:
        """
        Evaluate actions according to the current policy,
        ...
        if self.share_features_extractor:
            ...
            pi_features, vf_features = features
        latent_pi, lstm_states_temp = self._process_sequence(pi_features, lstm_states.pi, episode_starts, self.lstm_actor)
        if self.lstm_critic is not None:
            latent_vf, lstm_states_temp = self._process_sequence(vf_features, lstm_states.vf, episode_starts, self.lstm_critic)
        elif self.shared_lstm:
            latent_vf = latent_pi.detach()
        else:
            latent_vf = self.critic(vf_features)
        ...
        return values, log_prob, distribution.entropy(), lstm_states 
    

    # --------------------------------------------------------------------
    # def evaluate_actions(self, obs: PyTorchObs, actions: th.Tensor) -> Tuple[th.Tensor, th.Tensor, Optional[th.Tensor]]:
    #     # DEBUG 
    #     # BU FONKSIYON SB3 AIT evaluate_actions FONKSIYONU
    #     # LSTM ICINE KOPYALADIK
    #     # EKSIK / YANLISLAR GOZDEN GECIRILMELI !!!
    ...
    #         # NOTE: Features icerisinde inf degeri var ve "nan" cikti vermeye sebeb bu olabilir.
    #         # print("pi_features : ", features)
    #         latent_pi, latent_vf = self.mlp_extractor(features)

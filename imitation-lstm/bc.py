...
@dataclasses.dataclass(frozen=True)
class BehaviorCloningLossCalculatorDEBUG:
    ...
    def __call__(
        ...
        l2_norms = [th.sum(th.square(w.to(policy.device))) if not th.isnan(th.sum(th.square(w.to(policy.device)))) else th.zeros(1).to(policy.device) for w in policy.parameters()]
...
class BehaviorCloningLossCalculator():
    ...
    # ONES ? ZEROS - lstm her seferinde sifirlamamasi icin zeros olmalı. "done" durumunda sadece 1 olmali
    episode_starts = th.zeros(32,).cuda()

    _last_lstm_states = RNNStates(
        (
            th.zeros((1, 32, 256), device="cuda"),
            th.zeros((1, 32, 256), device="cuda"),
        ),
        (
            th.zeros((1, 32, 256), device="cuda"),
    ...
    def __call__(
        ...
        # DEBUG
        # sb3_contrib icine "policies.ActorCriticPolicy" ait evaluate_actions atinca
        # hata vermeden calisti
        # tabi basarim nasil / lstm kismi etkilenir mi kontrol edilmeli.

        
        
        # act, lstm_states_temp = policy.predict(obs, state=self.lstm_states, episode_start=self.episode_starts, deterministic=True)
        # # print(self.lstm_states[0].shape)
        # # print(self.lstm_states[1].shape)
        # lstm_states_temp = tuple(th.from_numpy(arr).to('cuda') for arr in lstm_states_temp if th.cuda.is_available())

        # pi_state = lstm_states_temp
        # vf_state = lstm_states_temp
        # self.lstm_states = RNNStates(pi=pi_state, vf=vf_state)



        if self.lstm_states == None:
            self.lstm_states = self._last_lstm_states
        
        # BU DEGERI INPUT OLARAK VERIP "policy.evaluate_actions" DAN YENI LSTM_STATE BILGISINI ALMALIYIZ 
        # SONRASINDA DONGU KENDINI TAMAMLAMIS OLUYOR

        # print(self.lstm_states)
        (_, log_prob, entropy, self.lstm_states) = policy.evaluate_actions_w_lstm_state(obs=tensor_obs, actions=acts, lstm_states=self.lstm_states, episode_starts=self.episode_starts)
        # (_, log_prob, entropy) = policy.evaluate_actions(obs=tensor_obs, actions=acts)
        ...
        # -------------------
        # DEBUG NAN DEGERLERI BASTAN DUZELTMEK ICIN 
        # test_l2 = [w if not th.isnan(w).any() else th.zeros_like(w) for w in policy.parameters()]
        # print("TEST L2: ", test_l2)
        # -------------------
        l2_norms = [th.sum(th.square(w)) for w in policy.parameters()]
        
        # -------------------
        # DEBUG bazi degerler nan oluyor, sum() yaptiktan sonra ise hepsi nan oluyor
        # nan olan degerleri 0 ile degistir
        # l2_norms = [th.sum(th.square(w)) if not th.isnan(th.sum(th.square(w))) else th.zeros(1) for w in policy.parameters()]
        l2_norms = [th.sum(th.square(w.to(policy.device))) if not th.isnan(th.sum(th.square(w.to(policy.device)))) else th.zeros(1).to(policy.device) for w in policy.parameters()]
        # -------------------
    ...
    def train(
        ...
        *,
        ...
        for (
            ...
            # DEBUG
            # loss_calculator girdileri dogru mu?
            # anlamak icin normal model egitimindeki girdiler ile karsilastir
            # DEBUG - LSTM  --> Hidden lstm_state ve episode_starts input olarak verilebilir mi ?
            ...
            # # DEBUG - Save policy
            # import os
            # if log_interval is not None and batch_num % log_interval == 0:
            #     policy_path = "/home/gazi13/catkin_ws_agile/src/agile_flight/envtest/saved_imit/Policy_imitation"
            #     os.makedirs(policy_path, exist_ok=True)
            #     self.policy.save(policy_path + "/iter_{0:05d}.pth".format(batch_num))
            #     print("SAVED SAVED SAVED SAVED ",policy_path)
            #     # self.venv.save_rms(
